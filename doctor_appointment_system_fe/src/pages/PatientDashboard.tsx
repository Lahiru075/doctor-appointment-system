...
const PatientDashboard = () => {
  ...
  const myStats = [
    { label: 'Total Sessions', value: totalBookings, status: 'Overall Bookings', color: 'text-indigo-600 bg-indigo-50 border-indigo-100' },
    { label: 'Upcoming Meets', value: upcomingCount, status: 'Active Duty', color: 'text-teal-600 bg-teal-50 border-teal-100' },
    { label: 'Completed Care', value: completedCount, status: 'Discharged', color: 'text-emerald-600 bg-emerald-50 border-emerald-100' },
    { label: 'Cancelled Sessions', value: cancelledCount, status: 'Inactive', color: 'text-rose-600 bg-rose-50 border-rose-100' },
  ];
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="bg-gradient-to-br from-[#8eb5ca] via-[#709eb7] to-[#082e3e] rounded-[2.5rem] border border-white p-8 md:p-10 relative overflow-hidden text-white shadow-lg">
        ...
        <div className="relative z-10 max-w-2xl">
          ...
          <h2 className="text-3xl md:text-4xl font-extrabold tracking-tight">
            Let's check your wellness guide, {user?.fullName || "Patient"}
          </h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 space-y-6">
          <div>
            ...
            <h3 className="text-xs font-extrabold uppercase tracking-widest text-[#082e3e]/70 mb-4 flex items-center gap-2">
              <Activity className="w-4 h-4 text-[#0a4053]" /> My Clinic Activity Summary
            </h3>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {myStats.map((stat, index) => (
                ...
                >
                  ...
                  <span className="text-2xl font-black text-[#082e3e] mt-2 block">
                    {stat.value}
                  </span>
                  <span className={`inline-flex items-center justify-center text-[9px] font-black px-3 py-1 rounded-full mt-3.5 mx-auto border ${stat.color}`}>
                    {stat.status}
                  </span>
                </motion.div>
              ))}
          ...
          {/* Next Consultation Summary Box (DYNAMIC) */}
            ...
            {isLoading ? (
              ...
              <div className="bg-[#f8fafc] border border-slate-200/40 rounded-[1.5rem] p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 relative overflow-hidden">
                ...
                <div className="space-y-1 pl-2 text-left">
                  <h4 className="font-extrabold text-[#082e3e] text-sm">{upcomingAppointment.doctorName}</h4>
                  <p className="text-xs text-slate-500 font-bold">{upcomingAppointment.specializationName} • Active Duty</p>
                ...
                <div className="bg-white px-4 py-2.5 rounded-xl border border-slate-200/50 text-right shrink-0 w-full sm:w-auto flex flex-row sm:flex-col justify-between items-center sm:items-end">
              ...
              <div className="border border-dashed border-slate-250 p-6 rounded-[1.5rem] flex flex-col sm:flex-row justify-between items-center gap-4 text-center sm:text-left bg-slate-50/50">
                <div>
                  <h4 className="font-extrabold text-[#082e3e] text-sm">No Upcoming Consultations</h4>
                  <p className="text-slate-400 text-xs font-semibold mt-0.5">Need medical counsel? Book a certified specialist today.</p>
                </div>
                <button onClick={() => navigate('/patient-book')} className="flex items-center gap-1 text-xs font-black text-[#0a4053] bg-white border border-slate-200 hover:bg-slate-50 px-4 py-2 rounded-xl shrink-0 cursor-pointer shadow-3xs">
        ...
        <div className="lg:col-span-4 space-y-6">
          {/* Action launcher */}
          <div className="bg-white border border-slate-200/50 rounded-[2rem] p-6 shadow-xs flex flex-col justify-between min-h-[170px]">
          ...
          <div className="bg-red-50 border border-red-100 rounded-[2rem] p-6 text-red-900 text-left">
